...
# 🔥 引入自動下載並在記憶體中合併歷史與最新資料的模組
from prepare_data import get_merged_dataframe

# ==========================================
# ⚙️ 參數設定 (加入 data/ 路徑，適應 GitHub Actions)
# ==========================================
OUTPUT_CSV = 'data/nba_advanced_injury_features.csv'
...
def generate_features():
    ...
    # 強制大寫保護
    df_adv_full.columns = [c.upper() for c in df_adv_full.columns]
    ...
    df_base_full.columns = [c.upper() for c in df_base_full.columns]
    ...
    df_stats['GAME_DATE'] = pd.to_datetime(df_stats['GAME_DATE'].str[:10])
    ...
    for m in metrics:
        df_stats[f'rust_adj_{m}'] = df_stats[f'{m}_r20'].fillna(0) * df_stats['rust_factor']
    ...
    active_rust_stats = active_rust_stats.rename(columns={col: f'active_{col}' for col in rust_metrics})
    ...
    lookup_df = df_stats[lookup_cols].dropna(subset=['GAME_DATE']).sort_values('GAME_DATE')
    
    # 🔥 關鍵修復：強制將 inactive_players 的欄位轉為大寫
    inactive_full = get_merged_dataframe("inactive_players")
    inactive_full.columns = [c.upper() for c in inactive_full.columns]
    
    inactive = inactive_full[['GAME_ID', 'TEAM_ID', 'PLAYER_ID']].copy()
    inactive = inactive.rename(columns={'GAME_ID': 'game_id', 'TEAM_ID': 'team_id'})
    
    # 🔥 關鍵修復：強制將 games 的欄位轉為小寫
    games_full = get_merged_dataframe("games")
    games_full.columns = [c.lower() for c in games_full.columns]
    
    games = games_full[['game_id', 'date']].copy()
    games = games.rename(columns={'date': 'GAME_DATE'})
    games['GAME_DATE'] = pd.to_datetime(games['GAME_DATE'].str[:10])
    
    inactive = inactive.merge(games, on='game_id', how='left')
    inactive = inactive.dropna(subset=['GAME_DATE']).sort_values('GAME_DATE')
    ...
    missing_stats = missing_stats.rename(columns={col: f'missing_{col}' for col in agg_dict.keys()})
    ...
    def merge_side_features(df, feat_df, side, prefix):
        ...
        df = df.merge(
            feat_df, left_on=['game_id', side], right_on=['game_id', 'TEAM_ABBREVIATION'], how='left'
        )
